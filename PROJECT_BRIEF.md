...
- **Pas de mempool public sur Arc** (`eth_subscribe("newPendingTransactions")` désactivé, confirmé en direct pendant la conception) → élimine structurellement le **sniping** : sur une chaîne à mempool visible (Ethereum et la plupart des L2), un bot peut voir la transaction d'achat de quelqu'un en attente, la copier avec un gas plus élevé, et lui voler l'affaire au prix qu'il visait — un problème réel et documenté sur les vrais drops (sneakers, NFT). Sur Arc, cette classe d'attaque est impossible.
